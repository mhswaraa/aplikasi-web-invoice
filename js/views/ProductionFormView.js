// ─────────────────────────────────────────────────────────────────────────────
// ProductionFormView (lokasi: /js/views/ProductionFormView.js)
// ─────────────────────────────────────────────────────────────────────────────
...
export const ProductionFormView = {
  render() {
    ...
    const rowsHTML = prod.items.map((it, idx) => `
      <tr class="${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}">
        ...
        <td class="px-4 py-2">
          <input
            type="number" name="qtyJadi" min="0"
            value="${it.qtyJadi||0}" required
            class="w-20 px-2 py-1 border rounded text-right focus:ring focus:ring-teal-200"
          />
        </td>
        <td class="px-4 py-2">
          <input
            type="number" name="defect" min="0"
            value="${it.defect||0}" required
            class="w-20 px-2 py-1 border rounded text-right focus:ring focus:ring-teal-200"
          />
        </td>
      </tr>
        ...
        <form id="production-form">
          ...
          <div class="mb-6">
            ...
            <select id="status" name="status"
              class="w-full px-3 py-2 border rounded focus:ring focus:ring-teal-200"
            >
              ${['pending','in_progress','done'].map(s => `
                <option value="${s}" ${prod.status===s?'selected':''}>
                  ${s.replace(/_/g,' ')}
                </option>`).join('')}
            </select>
          </div>
  ...
  afterRender(prod) {
    ...
    form.addEventListener('submit', e => {
      ...
      // reload agar tombol invoice tampil jika status done
      this.render();
    });
